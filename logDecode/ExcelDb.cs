...
namespace DRT
{
	// source: http://www.cnblogs.com/yiyumeng/archive/2012/01/19/2327469.html
    class ExcelDb
    {
        #region 取文件的擴展名 [1]
        /// <summary>
        ...
        #region 檢查一個文件是不是2007版本的Excel文件 [1]
        /// <summary>
        ...
        public static String GetExcelConnectionString(string excelFile, bool no_HDR)
        {
            ...
            try
            {
                if (no_HDR)
                {
                    ...
                    {
                        return "Provider=Microsoft.Ace.OleDb.12.0;" + "data source=" + excelFile + ";Extended Properties='Excel 12.0; HDR=NO; IMEX=1'"; //此?接可以操作.xls与.xlsx文件
                    }
                    else
                    {
                        return "Provider=Microsoft.Jet.OleDb.4.0;" + "data source=" + excelFile + ";Extended Properties='Excel 8.0; HDR=NO; IMEX=1'"; //此?接只能操作Excel2007之前(.xls)文件
        ...
        public static String GetExcelConnectionString(string excelFile)
        {
            try
            {
                if (IsExcel2007(excelFile))
                {
                    return "Provider=Microsoft.Ace.OleDb.12.0;" + "data source=" + excelFile + ";Extended Properties='Excel 12.0;  IMEX=1'"; //此?接可以操作.xls与.xlsx文件
                }
                else
                {
                    return "Provider=Microsoft.Jet.OleDb.4.0;" + "data source=" + excelFile + ";Extended Properties='Excel 8.0;  IMEX=1'"; //此?接只能操作Excel2007之前(.xls)文件
        ...
        public static String GetExcelConnectionStringByWrite(string excelFile)
        {
            try
            {
                if (IsExcel2007(excelFile))
                {
                    return "Provider=Microsoft.Ace.OleDb.12.0;" + "data source=" + excelFile + ";Extended Properties='Excel 12.0;'"; //此?接可以操作.xls与.xlsx文件
                }
                else
                {
                    return "Provider=Microsoft.Jet.OleDb.4.0;" + "data source=" + excelFile + ";Extended Properties='Excel 8.0;'"; //此?接只能操作Excel2007之前(.xls)文件
        ...
        #region 讀取Excel中的所有表名 [3]
        //讀取Excel中的所有表名
        //?取Excel文件?，可能一?文件中?有多?Sheet，因此?取Sheet的名?是非常有用的

        /// <summary>
        /// 根据Excel物理路??取Excel文件中所有表名,列名是TABLE_NAME
        /// </summary>
        /// <param name="excelFile">Excel物理路?</param>
...
/*                        if (ss.LastIndexOf('$') > 0)
        ...
        #region Sheet2DataTable [2]
        /// <summary>
        /// 獲取Excel文件中指定SheetName的內容到DataTable
        /// </summary>
        /// <param name="FileFullPath">Excel物理路?</param>
...
//                DataSet ds = new DataSet();
                ...
                {
//                    string colName;
                    if (colName.Substring(colName.Length - 2) == "$_")
                        continue;
/*                    if (colName0.Substring(colName0.Length - 1) == "$")
                        colName = colName0.Substring(0, colName0.Length - 1);
                    else
                        colName = colName0;*/
// TODO: don't know why, but need to skip something like xxxx$_ after xxx$
                    if (!(colName == prevName + "_"))
                    {
                        OleDbDataAdapter odda = new OleDbDataAdapter(string.Format("SELECT * FROM [{0}]", colName), conn);                    //("select * from [Sheet1$]", conn);
                        odda.AcceptChangesDuringFill = false;
        ...
        #region 在Excel文件中創建表,Excel物理路徑如果文件不是一個已存在的文件，會自動創建文件
        /// <summary>
        /// 在一個Excel文件中創建Sheet
        /// </summary>
        /// <param name="servepath">Excel物理路?,如果文件不是一個已存在的文件，會自動創建文件</param>
        ...
        public static bool CreateSheet(string servepath, string sheetName, string[] cols)
        {
            try
            {
                ...
                if (cols.Equals(null))
                {
                    throw new Exception("創建表需要提供欄位列表。");
                }
        ...
        #region DataTable2Sheet,把一個DataTable寫入Excel中的表,Excel物理路徑,如果文件不是一個已存在的文件，會自動創建文件
        /// <summary>
                    ...
                    using (OleDbConnection conn = new OleDbConnection(GetExcelConnectionStringByWrite(servepath)))
                    {
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            if (maxrow == 0)//不需要限制一個表的行數
                            {
                                ...
                                cmd.CommandText = strSQL.Append(" insert into [" + sheetName + "]( ")
                                .Append(strfield.ToString()).Append(") values (").Append(strvalue /*.Replace("'","''")*/
                                ).Append(")").ToString();
                            }
                            else
                            {
